...
#pireblog_parse_v4.py - 02-27-11 JG
...
def printEntriesHTML(tag, entries):
    current_ssi = file.open('%s.ssi' % tag,w)
    ...
    current_ssi.write('<h2>%s:</h2>\n' % (tagcapped))
    for entry in entries:
        current_ssi.write('<h3>%s - <a href="%s">%s</a></h3>\n' % (entry['date'].strftime('%B %d, %Y'),entry['link'],entry['title']))
        if entry['image'] != "No Image":
            current_ssi.write(print '<img src="%s"/>' % entry['image'])
    current_ssi.close()
