...
def printEntriesHTML(tag, entries,num=''):
    ''' 
...
if "printStyle" not in args: 
    ...
    printStyle = str(args["numEntries"].value)
...
#I added a line up top that makes this next part unnecessary, because tagList will always be a list -JG
...
for tag in tagList:
    printEntriesHTML(tag, tagDict[tag],numEntries)
